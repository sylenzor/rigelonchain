...
Seven MCP tools (Zod-validated inputs), v1.2.0:
...
3. `rigel_analyze_rug_risk(tokenAddress)` — mint/freeze authority, top-holder concentration → risk report
4. `rigel_check_first_buyers(tokenAddress)` — bundle detection: fresh-wallet holders, shared funders, clusters
5. `rigel_deployer_history(tokenAddress)` — deployer rap sheet: who launched it, wallet age/activity, remaining bag
6. `rigel_market_snapshot(tokenAddress)` — DexScreener: price, mcap, liquidity, volume, buy/sell flow, momentum
7. `rigel_full_audit(tokenAddress)` — all checks in one call, combined verdict + top reasons
...
- **MCP server: BUILT at mcp/** — TypeScript ESM on @modelcontextprotocol/sdk + @solana/web3.js. 7 tools (see list above), Zod inputs, StdioServerTransport, stderr-only logging, friendly pubkey/RPC errors, pump.fun bonding-curve + Metaplex metadata parsing. Verified: tsc clean, MCP initialize handshake + tools/list + error path tested over stdio. `npm run build` → dist/.
